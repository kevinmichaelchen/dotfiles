...
Tools that are **development/language runtimes** → Migrate to Mise
Tools that are **system-level packages** → Keep in Nix/Homebrew
...
4. **Rebuild your system**:
   ...
   - `home-manager switch`
5. **Verify all tools are accessible**: Run `which <tool>` for each migrated tool
...
### PATH Integration
Mise automatically manages PATH when activated. Ensure `mise activate` is in your shell config:
```bash
eval "$(mise activate zsh)"
```

This is already managed via Chezmoi in `~/.config/zsh/custom.zsh` or your shell initialization.
